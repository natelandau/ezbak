...
def main(cmd: EZBakCLI) -> None:
    ...
    try:
        ...
        if outcome is RestoreOutcome.NO_BACKUP:
            ...
            # --skip-if-no-backup, a missing backup is a clean no-op so a pre-start restore
            # never blocks a first deployment.
            if app.settings.skip_if_no_backup:
                logger.info("No backup matched and --skip-if-no-backup is set; nothing to restore")
                return
